...
export default {
  ...
  async created() {
    ...
    this.activePanel = [
      { text: 'front', arr: this.UnitsArr.filter((e) => e.place === 1) },
      { text: 'mid', arr: this.UnitsArr.filter((e) => e.place === 2) },
      { text: 'back', arr: this.UnitsArr.filter((e) => e.place === 3) },
    ]
  },
  methods: {
    async search() {
    ...
    onCharaSelect(e, s) {
      ...
      this.charSort = []
      for (let i = 0; i < this.charSelect.length; i++) {
        this.charSort.push(this.findChar(this.charSelect[i]))
      }
      const sortListChar = this.charSort.sort((a, b) => this.reverseCompare(a, b)).filter((e) => !this.th.has(e)).reverse()
      this.charSelect = []
      for (let j = 0; j < sortListChar.length; j++) {
